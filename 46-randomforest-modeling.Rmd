...
pacman::p_load(glmnet, tictoc, vip, tidytext, randomForest, doParallel)
registerDoParallel()
```
...
rf_grid <- grid_max_entropy(rf_parameters, size=2)
```
...
rf_vip <- rf_final_fit %>%
  pull_workflow_fit() %>%
  vi_model(lambda = .$spec$args$min_n) %>%
...
rf_vip %>%
  ...
  labs(title='Scaled coefficient magnitudes of glmnet',
       subtitle=str_c('Min_n:', format(pull_workflow_fit(rf_final_fit)$spec$args$min_n,
                                         digits=5, nsmall=2, scientific=TRUE),
                      'Trees:', format(pull_workflow_fit(rf_final_fit)$spec$args$trees[[2]],
                                         digits=5, nsmall=3),
                      sep=' '),
