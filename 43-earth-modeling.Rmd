...
In this notebook, we use earth to perform modeling.  Our general approach will be to use hyperparameter tuning via cross-validation to identify the best performing hyperparameters,  In another notebook, we will investigate the performance of the model.
...
pacman::p_load(glmnet, tictoc, vip, tidytext, doParallel, earth)
library(discrim)
registerDoParallel()
```
...
use_earth(particle_class ~ ., data=train_data)
```
...
earth_recipe <- 
  ...
  step_string2factor(one_of(id, img_id, filter0, filter1, filter2, filter3, filter4, 
    filter5, filter6)) %>% 
...
earth_spec <-
  earth::earth( x = train_data$particle_class, y = train_data$particle_class) %>% #no x value??
  set_mode("classification") %>%
  set_engine("glmnet") 
