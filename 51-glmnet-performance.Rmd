...
glmnet_pred_helper<-function(object, newdata, ...){
  predict(object, newx=as.matrix(newdata), ...)
}
glmnet_vip <- plot_variable_importance(glmnet_final_fit, newdata=train_data, mdl_name = 'glmnet',
                                       positive_class='site',
                                       pred_wrap=glmnet_pred_helper,
                                       lambda=glmnet_final_fit$fit$fit$spec$args$penalty)
```
