...
pacman::p_load(glmnet, tictoc, vip, tidytext, doParallel, caret)
library(discrim)
```
...
nb_grid <- grid_max_entropy(nb_parameters, size=2) #THIS SHOULD BE 20
```
...
What is of interest here is the color of the squares.  Red indicates that the performance is poor, and green indicates that the performance is great.
...
best_nb_params
```
 We can see that Model 3 (best in overall rank and mean metric performance) predictably had the highest `pr_auc`. 
