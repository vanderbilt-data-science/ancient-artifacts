...
pacman::p_load(glmnet, tictoc, vip, tidytext, xgboost, recipes)
registerdoParallel()
```
...
xgb_grid <- grid_max_entropy(xgb_parameters, size=2)
```
...
xgb_vip %>%
  ...
  labs(title='Scaled coefficient magnitudes of glmnet',
       subtitle=str_c('eta:', format(pull_workflow_fit(xgb_final_fit)$spec$args$eta,
                                         digits=5, nsmall=2, scientific=TRUE),
                      'gamma:', format(pull_workflow_fit(xgb_final_fit)$spec$args$gamma[[2]],
                                         digits=5, nsmall=3),
                      sep=' '),
...
```{r save markdown}
fs::file_copy('41-glmnet-modeling.nb.html', './html_results/41-glmnet-modeling.nb.html', overwrite=TRUE)
```
