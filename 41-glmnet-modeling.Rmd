...
pacman::p_load(glmnet, tictoc, tidytext)
```
...
#glmnet_grid <- grid_regular(glmnet_parameters, levels=5)
glmnet_grid <- grid_max_entropy(glmnet_parameters, size=30)
```
...
```{r save glmnet model to box}
#move_model_info('glmnet', 'save', box_dir = path.expand('~/../Box/DSI_AncientArtifacts/'))
```
