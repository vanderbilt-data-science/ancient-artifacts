...
# Variable Importance
**Note about the following function:** There is a real oddity about the behavior of `vi_permute`.  Providing a single interface like this to a set of functions which have differing implementations somewhat has resulted in some oddball behavior.  The primary oddity is as follows:
...
plot_variable_importance <- function(wkflw, newdata, mdl_name=NULL, pred_wrap=predict, plot_type='default',
  ...
  baked_data <- bake(mdl_recipe, newdata)
  ...
  if(names(list(...))=='lambda')
  {
    lambda_val = list(...)[['lambda']]
    pred_wrapper <- function(object, newdata, lambda=eval(lambda_val))
    {
      pred_wrap(object, newdata, lambda=lambda, ...)
    }
  }
  
  #get permutation importance
  vip_res <- vi_permute(object=mdl_fit, train=prepped_data, target=target,
